...
### 포트 배치 (단일 소스: launcher/robot/config/robot.yaml)
...
| 9876 | robot PC | 런처 웹 대시보드 HTTP (launcher.robot.web) |
...
│
├── launcher/                          # 로봇 PC 런처 (self-contained — teleop_dev import 없음)
│   ├── config.py / process.py         #   yaml 컴포넌트 정의 + subprocess 생명주기 (코어)
│   ├── manager.py                     #   LocalManager (위상 정렬 start_all/stop_all)
│   └── robot/                         #   로봇 PC 전용
│       ├── bringup.py                 #     터미널 브링업 CLI (--side right|left|both)
│       ├── web.py / api.py            #     웹 대시보드 데몬 (HTTP 9876) + JSON API
│       ├── parts.py / status.py       #     파츠 정의 + 상태 수집 (probe/로그 파싱)
│       ├── actions.py / move_home.py  #     E-Stop burst + 초기 위치 이동 oneshot
│       ├── params.py                  #     파라미터 화이트리스트 편집 (주석 보존)
│       ├── assets/                    #     index/arm/cam HTML + app.js (framework-free)
│       └── config/robot.yaml          #     로봇 PC 구성 단일 소스 (+demo.yaml 검증용)
│
├── docs/                              # 문서
...
> 로봇 PC 구성요소는 웹 대시보드로 관리: robot PC 에서
> `python3 -m launcher.robot.web` → 조종 PC 브라우저 `http://<robot_ip>:9876/`
> (터미널만 쓸 땐 `python3 -m launcher.robot.bringup --side both`).
> 상세: [docs/launcher_guide_ko.md](launcher_guide_ko.md). 아래는 개별/수동 실행.
