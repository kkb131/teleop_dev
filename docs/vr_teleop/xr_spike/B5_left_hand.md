...
**원본 (final, 사용자 hardware 정상 동작 확인)**: [tesollodelto/tesollo_model dg5f_left.urdf](https://github.com/tesollodelto/tesollo_model/blob/main/dg5f/dg5f_left.urdf) (745 lines).

⚠️ **B5 진행 중 URDF 출처 + 수정 두 차례 변경 — final 상태 확정 (2026-05-27)**:

| 단계 | URDF 출처 | 우리 수정 | 결과 |
|---|---|---|---|
| B5 초기 (commit `96e3ee1`) | `dg5f_ros2/dg5f_description/urdf/dg5f_left.urdf` | (A) mesh path → `./meshes/`, (B) PIP/DIP `lower=0`, 양쪽 검증 | left 4 손가락 OK, **엄지 거의 안 움직임** (사용자 1차 보고) |
| B5 1차 fix (commit `6d92185`) | 같음 | + (C) 4 single-sign joint limit right 와 sign 통일 시도 | **새끼손가락 더 나빠짐 + 엄지 전혀 안 움직임** (사용자 2차 보고) — 1차 가설 오답 |
| B5 revert (commit `c31cad2`) | 같음 | 4 sign 변경만 revert (PIP/DIP `lower=0` 유지) | 새끼손가락 회복, 엄지는 여전히 안 움직임 |
| **B5 final (현재)** | **tesollodelto/tesollo_model dg5f_left.urdf** | **mesh path 만 분리 폴더로 조정** | **사용자 hardware 정상 동작 확인 (엄지 포함)** |

#### B5 final 수정 (mesh path 만):

새 URDF 의 mesh 라인:
```
package://meshes/dg5f_left/ll_dg_*.dae   (visual)
package://meshes/dg5f_left/ll_dg_*_c.STL (collision)
```

우리 폴더 구조 (`./meshes/visual/` + `./meshes/collision/`) 에 맞춰 sed 일괄 치환:
```diff
- package://meshes/dg5f_left/(*.dae)
+ ./meshes/visual/\1
- package://meshes/dg5f_left/(*.STL)
+ ./meshes/collision/\1
```

→ visual = .dae 파일 (28 개), collision = .STL 파일 (28 개). right URDF 와 같은 분리 폴더 패턴.

#### B5 진행 중 원복된 수정들

이전에 했던 수정들 모두 **새 URDF 채택과 동시에 원복**:

1. **PIP/DIP `lower=0` modification** — 원복. 새 URDF 의 PIP/DIP (`lj_dg_*_{3,4}`) 가 native `[-π/2, π/2]` 양방향 그대로 보존. retarget URDF 가 modification 없이 그대로 동작.
2. **4 single-sign joint limit (`_1_1`, `_1_2`, `_5_1`, `_5_2`)** — 이미 c31cad2 에서 revert. 새 URDF 도 mirror sign 그대로 (단, 일부 limit 값이 dg5f_ros2 와 다름 — 예: `_1_2` upper=2.71 (≈155°) vs dg5f_ros2 의 π (180°)). 사용자 hardware 와 정확히 일치.

#### Key joint limit 비교 (final state)

| joint | tesollodelto (new, final) | dg5f_ros2 (old) | 차이 |
|---|---|---|---|
| `lj_dg_1_1` (thumb abd) | `[-1.34, 0.38]` | `[-0.89, 0.38]` | lower 더 넓음 |
| `lj_dg_1_2` (thumb opp) | `[0, 2.71]` | `[0, π]` | upper 더 좁음 (155° vs 180°) |
| `lj_dg_1_3, _1_4` (thumb PIP/DIP) | `[-π/2, π/2]` | `[-π/2, π/2]` | 동일 |
| `lj_dg_5_1` (pinky abd) | `[-1.05, 0.0]` | `[-1.05, 0.017]` | upper 약간 좁음 |
| `lj_dg_5_2` | `[-π/2, 0.262]` | `[-0.61, 0.42]` | 완전 다름 |
| `lj_dg_*_2` (MCP flex) | `[0, ~]` | `[0, 2.0]` | 비슷 |

새 URDF 가 hardware 의 정확한 motion envelope 을 반영 — 사용자 hardware 정상 동작이 그 증거.
...
| **엄지가 안 굽혀짐 (다른 손가락 OK)** | **B5 final 에서 해결됨** — dg5f_ros2 native URDF 가 hardware envelope 과 안 맞아 thumb opp limit 등 부정확. [tesollodelto/tesollo_model](https://github.com/tesollodelto/tesollo_model/blob/main/dg5f/dg5f_left.urdf) 의 URDF 가 정확한 hardware envelope (예: `lj_dg_1_2 [0, 2.71]` 등) — 사용자 hardware 정상 동작 확인 | dg5f_left_retarget.urdf 를 tesollodelto/tesollo_model 의 dg5f_left.urdf 로 교체 + mesh path 만 수정 |
