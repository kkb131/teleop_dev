...
python3 -m sender.arm.calibrate --output vive/calibration.json
```
...
```bash
python3 -m sender.arm.calibrate --verify vive/calibration.json
```
...
# 트래커 목록 확인
python3 -m sender.arm.vive_tracker --list

# 실시간 포즈 확인
python3 -m sender.arm.vive_tracker --hz 50
```
...
# 캘리브레이션 적용
python3 -m sender.arm.vive_sender --target-ip <ROBOT_PC_IP> --calibration vive/calibration.json

# 특정 트래커 지정
python3 -m sender.arm.vive_sender --target-ip 192.168.0.10 --tracker-serial LHR-XXXXXXXX
```
...
- `python3 -m sender.arm.vive_tracker --list`로 디바이스 확인
...
```bash
python3 -m sender.arm.tests.test_step1_openvr
```
```
# 예상 출력:
...
```bash
python3 -m sender.arm.tests.test_step2_pose --duration 3
```
```
# 예상 출력:
...
python3 -m sender.arm.tests.test_step3_udp --port 9873

# 수동 모드 (두 터미널)
# Terminal 1 - 수신
python3 -m sender.arm.tests.test_step3_udp --mode recv --port 9873
# Terminal 2 - 송신
python3 -m sender.arm.tests.test_step3_udp --mode send --target-ip 127.0.0.1 --port 9873
```
```
# 예상 출력:
...
```bash
python3 -m sender.arm.tests.test_step4_calibration
```
```
# 예상 출력:
...
```bash
python3 -m sender.arm.tests.test_step5_velocity --port 9872
```
```
# 예상 출력:
...
```bash
# 조종 PC
python3 -m sender.arm.vive_sender --target-ip <ROBOT_PC_IP> --port 9871
