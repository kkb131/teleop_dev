...
⚠️  Chrome 창 / 헤드셋 orientation 이 robot base_link 의 frame 기준:
    WebXR local-floor reference space 의 +x/+y/+z 는 사용자가 Chrome 창을
    띄울 때 헤드셋의 정면 시선 방향 으로 정의됨. 즉 'r' 캘리브레이션 시점에
    헤드셋이 향하던 방향이 robot base_link 의 정면 (+x) 과 align 되어야
    위 매핑이 의도대로 동작. 헤드셋이 robot base_link 의 정면 (+x) 에서 N°
    회전한 자세로 Chrome 창을 띄우고 'r' 을 누르면 모든 손 motion 이 N° 만큼
    회전된 frame 으로 매핑돼 robot 거동이 의도와 N° 어긋남.
    실제 운영 시:
      - Chrome 창을 띄울 때 헤드셋 정면이 robot 의 정면 (+x base_link 방향) 을
        가리키도록 자세 잡고 시작.
      - 도중에 의도와 90°/180° 어긋난 거동이 보이면 헤드셋 자세부터 의심.
