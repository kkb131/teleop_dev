...
기본 설정 파일: `standalone/teleop_admittance/config/default.yaml`
...
```bash
# 편집
vi standalone/teleop_admittance/config/default.yaml

# 실행 (수정된 default.yaml 자동 로드)
python3 -m standalone.teleop_admittance.main --mode sim
```
...
```bash
# 복사
cp standalone/teleop_admittance/config/default.yaml \
   standalone/teleop_admittance/config/hand_guiding.yaml

# 편집
vi standalone/teleop_admittance/config/hand_guiding.yaml

# 실행
python3 -m standalone.teleop_admittance.main --config standalone/teleop_admittance/config/hand_guiding.yaml
```
...
```bash
python3 -m standalone.teleop_admittance.main --mode rtde --robot-ip 192.168.0.2
```
...
| 파일 | 역할 |
|------|------|
| `standalone/teleop_admittance/config/default.yaml` | 기본 설정 (전체 항목 + 주석) |
| `standalone/teleop_admittance/teleop_config.py` | YAML 로더 + dataclass 정의 (기본값 포함) |
| `standalone/teleop_admittance/main.py` (line 435~458) | CLI argparse 정의 |
| `standalone/core/compliant_control.py` | 어드미턴스 프리셋 M/D/K 값 |
